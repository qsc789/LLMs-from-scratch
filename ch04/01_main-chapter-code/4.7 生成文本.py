...
encoded = tokenizer.encode(start_context)# 编码数据
print("encoded:", encoded)

encoded_tensor = torch.tensor(encoded).unsqueeze(0)# 转换为tensor
print("encoded_tensor.shape:", encoded_tensor.shape)
model = GPTModel(GPT_CONFIG_124M)# 加载模型
model.eval() # disable dropout

out = generate_text_simple(# 调用生成文本函数
    model=model,
    idx=encoded_tensor,
    max_new_tokens=6,
...
decoded_text = tokenizer.decode(out.squeeze(0).tolist())# 解码输出
print(decoded_text)